...
Any script that modifies recipes should be placed in the <Folder>server_scripts/</Folder> folder, and can be reloaded at any time with <Cmd>/reload</Cmd>.
...
1. A filter, similar to the ones used when [removing recipes](https://kubejs.com/wiki/tutorials/recipes#removing-recipes)
2. The ingredient to replace
3. The ingredient to replace it with (can be a tag)

For example, let's say you were removing all sticks and wanted to make people craft things with saplings instead. Inside your [callback](https://kubejs.com/wiki/tutorials/recipes#recipes-callbacks-and-you) you would put:
