...
// Stolen From Fumadocs UI
// https://github.com/fuma-nama/fumadocs/blob/dev/packages/ui/src/components/tabs.tsx
...
>((props, ref) => (
  <Unstyled.TabsTrigger
    ...
    {...props}
    className={cn(
      'inline-flex items-center gap-2 whitespace-nowrap text-zinc-400 border-b border-transparent py-2 text-sm font-medium transition-colors [&_svg]:size-4 hover:text-zinc-200 disabled:pointer-events-none disabled:opacity-50 data-[state=active]:border-indigo-300 data-[state=active]:text-indigo-300',
      props.className,
    )}
  />
))
...
export function Tabs({
  ...
  return (
    <Unstyled.Tabs
      ref={ref}
      className={cn(
        'flex flex-col overflow-hidden rounded-xl border border-zinc-800 bg-zinc-900 my-4',
        className,
      )}
      value={value}
...
export function TabsContent({
  ...
  return (
    <Unstyled.TabsContent
      ...
      forceMount
      className={cn(
        'p-0 text-[15px] bg-zinc-800 rounded-xl outline-none prose-no-margin data-[state=inactive]:hidden [&>figure:only-child]:-m-4 [&>figure:only-child]:border-none',
        className,
      )}
      {...props}
    >
